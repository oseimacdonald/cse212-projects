public static class Trees
{
    ...
    if (first > last)
    {
        return; // Base case: if the range is invalid, stop recursion
    }

    int mid = (first + last) / 2;
    bst.Insert(sortedNumbers[mid]); // Insert the middle element
    
    // Recursively insert the middle element from the left and right halves
